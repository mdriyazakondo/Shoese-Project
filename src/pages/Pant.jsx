...
const Pant = () => {
  ...
  const [products, setProducts] = useState(pantData || []);
  const [selectedCategory, setSelectedCategory] = useState("All");
  const [searchTerm, setSearchTerm] = useState("");

  useEffect(() => {
    let filteredProducts = pantData || [];
  ...
  }, [selectedCategory, pantData, searchTerm]);
    ...
    <div className="pt-10 bg-gray-50 min-h-[93vh]">
      <div className="max-w-7xl mx-auto px-5 md:px-0">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {products.length > 0 ? (
            products.map((item) => <PantSection key={item.id} item={item} />)
          ) : (
